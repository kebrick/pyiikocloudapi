...
from pyiikocloudapi.decorators import experimental, deprecated
...
from pyiikocloudapi.models import OrganizationsModel, ErrorModel, CouriersModel, BaseResponseModel, ByIdModel, \
    ...
    BaseStreetByCityModel, BaseTerminalGroupsModel, BaseTGIsAliveyModel
...
class BaseAPI:
    ...
    def __init__(self, api_login: str, session: Optional[requests.Session] = None, debug: bool = False):
        ...
        # https://take-eat-tso.iikoweb.ru/
        # https://api-ru.iiko.services/api/1/access_token
        self.__base_url = "https://api-ru.iiko.services"
        self.__headers = {
    ...
    def access_token(self):
        ...
        # result = self.session_s.post(f'{self.__base_url}/api/1/access_token', data={"apiLogin": self.api_login})
        ...
        try:
            ...
            if out.get("token", None) is not None:
                self.check_status_code_token(result.status_code)
                self.__token = out.get("token", "")
    ...
    def _post_request(self, url: str, data: dict, model_response_data=None, model_error=ErrorModel):
...
class TerminalGroup(BaseAPI):
    def terminal_groups(self, organization_ids: List[str], include_disabled: bool = False) -> Union[ErrorModel, BaseTerminalGroupsModel]:
        """
    ...
    def is_alive(self, organization_ids: List[str], terminal_group_ids: List[str]) -> Union[ErrorModel, BaseTGIsAliveyModel]:
        """
...
class Deliveries(BaseAPI):
    ...
    def update_order_delivery_status(self,
        ...
        try:
            #
            # result = self.session_s.post(f'{self.base_url}/api/1/deliveries/update_order_delivery_status',
    ...
    def cancel_confirmation(self,
        ...
        try:
            # result = self.session_s.post(f'{self.base_url}/api/1/deliveries/cancel_confirmation',
            #                              json=json.dumps(data), headers=self.headers)
            #
            # out: dict = json.loads(result.content)
            #
    ...
    def by_id(self,
        ...
        try:
            # result = self.session_s.post(f'{self.base_url}/api/1/deliveries/by_id', json=json.dumps(data),
            #                              headers=self.headers)
            #
            # out: dict = json.loads(result.content)
            #
    ...
    def by_delivery_date_and_source_key_and_filter(self,
        ...
        try:
            # result = self.session_s.post(f'{self.base_url}/api/1/deliveries/by_delivery_date_and_source_key_and_filter',
            #                              json=json.dumps(data),
            #                              headers=self.headers)
            #
            # out: dict = json.loads(result.content)
...
class IikoTransport(Orders, Deliveries, Employees,Street,TerminalGroup):
    pass
